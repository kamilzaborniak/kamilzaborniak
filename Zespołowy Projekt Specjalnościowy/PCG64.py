...
def ParseArguments():
    ...
    parser.add_argument('--n', default="200", required=False, help='number of generated numbers (default: %(default)s)')
...
'''
if len(seeds) == 1:
    PCG64 = np.random.default_rng(np.random.PCG64(seed=int(seeds)))
    pcg64_1 = PCG64.integers(size=n, low=0, high=2 ** 31-1)
    if output_file == "":
        print("Wygenerowane liczby: \n", pcg64_1)
    else:
        data = {'PRNG': "PCG64",
    ...
    for nr, seed in enumerate(seeds):
        #print(seed)
        PCG64 = np.random.default_rng(np.random.PCG64(seed=int(seed)))
        print(nr)
